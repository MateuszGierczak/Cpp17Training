...
{
    REQUIRE_FALSE(std::is_same<decltype(non_noexcept_function), decltype(noexcept_function)>::
                      value); // Before cpp17 type of these two functions was same
    ...
    // void (*ptr_fun1)() noexcept = non_noexcept_function; //it doesn't work because noexcept
    // function pointer can't bind non noexcept function
}
